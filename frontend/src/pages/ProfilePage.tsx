import { Link } from "react-router-dom";
import { PageHeader } from "@/components/PageHeader";
import { Button } from "@/components/ui/button";
import { Card } from "@/components/ui/card";
import { demoProfile } from "@/lib/demoData";

const fields = [
  ["Name", demoProfile.name],
  ["Email", demoProfile.email],
  ["College", demoProfile.college],
  ["Course", demoProfile.course],
  ["Semester", `Semester ${demoProfile.semester}`],
  ["Section", demoProfile.section],
] as const;

export function ProfilePage() {
  return (
    <div className="mx-auto max-w-3xl min-w-0">
      <PageHeader
        eyebrow="Account"
        title="Profile"
        description="Demo student record. Editing will wait for authentication."
        action={
          <Link to="/app/settings">
            <Button variant="outline" size="sm">
              Settings
            </Button>
          </Link>
        }
      />
      <Card>
        <div className="flex flex-col items-start gap-5 sm:flex-row">
          <div className="grid size-24 place-items-center rounded-[1.75rem] bg-[linear-gradient(145deg,#5b4bff,#8f84ff)] text-2xl font-semibold text-white">
            {demoProfile.initials}
          </div>
          <div className="min-w-0 flex-1">
            <p className="font-display text-2xl">{demoProfile.name}</p>
            <p className="text-sm text-muted">{demoProfile.email}</p>
            <div className="mt-5 grid gap-3 sm:grid-cols-2">
              {fields.map(([label, value]) => (
                <div key={label} className="rounded-2xl bg-canvas px-3 py-3">
                  <p className="text-xs text-muted">{label}</p>
                  <p className="mt-1 font-medium">{value}</p>
                </div>
              ))}
            </div>
          </div>
        </div>
      </Card>
    </div>
  );
}
